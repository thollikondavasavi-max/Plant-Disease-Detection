...
def analyze_plant_image(image_base64, disease, confidence):
...
The disease has ALREADY been predicted by a Deep Learning CNN model.

CNN Prediction:
Disease: {disease}
Confidence: {confidence:.2f}%

Your job is NOT to classify the disease again.

Do NOT change the disease name.

Only explain the prediction.

Return ONLY valid JSON.

{{
    condition = "Healthy" if "healthy" in disease.lower() else "Diseased"
    ...
    "note": "Explanation generated using the CNN prediction."
}}
"""
...
def analyze():
    ...
    disease, confidence = predict_disease(image_file)
    ...
    # Analyze with AI (ONLY ONCE)
        report = analyze_plant_image(image_base64, disease, confidence)
